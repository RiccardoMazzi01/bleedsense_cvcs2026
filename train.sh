...
#
# Uso: sbatch --job-name=<nome> train.sh <dataset> <architecture> <fold>
# Esempio: sbatch --job-name=hemoset_unet_f0 train.sh hemoset unet 0
#
# Per un test rapido (debug, priorita' alta, max 1h/1GPU) sovrascrivi
# partition/qos/time da riga di comando, es.:
#   sbatch --qos=all_qos_dbg --time=00:10:00 --job-name=debug train.sh hemoset unet 0
