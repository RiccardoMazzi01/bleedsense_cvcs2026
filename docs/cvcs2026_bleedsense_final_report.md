...
Automatic hemostasis management in robotic surgery requires reliable segmentation of blood in the operative field. Models trained on a single surgical dataset, however, tend to generalize poorly to other datasets with different visual characteristics. This work studies the issue on two public datasets, HemoSet (robotic surgery on a porcine model) and the dataset of Rabbani et al. (gynecological laparoscopy on humans), confirming an asymmetric domain gap already observed in previous work from the research group. Six families of strategies for reducing this gap are then compared systematically: aggressive data augmentation, appearance/domain adaptation (Reinhard, FDA), multi-domain joint training (with and without balancing), ensembling of multiple architectures, an illumination-invariant input channel, feature-level style mixing (MixStyle), and a test-time BatchNorm recalibration. Across three standard architectures (UNet, UNet++, DeepLabV3+, `resnet34` encoder), aggressive data augmentation is the most effective strategy: it improves cross-dataset robustness in both directions with no appreciable in-domain cost, and combined with an ensemble of the three architectures it yields the best overall result. The reasons behind the weaker performance of the remaining strategies are also analyzed.

---

## 1. Introduction and Motivation

Automating hemostasis management in robotic surgery — identifying and controlling bleeding during the procedure — requires, as a first step, reliable segmentation of blood in the endoscopic field of view. This project is part of the TRAMIS research initiative, focused on real-time bleeding detection in robotic surgery. Real, annotated TRAMIS surgical data were not yet available at the time of this work, so the study was conducted on two public reference datasets in the domain, used as proxies:
...
The three architectures, trained separately on the two datasets with minimal augmentation, confirm and quantify the asymmetry: for HemoSet→Rabbani, mean Dice drops by about 70% relative (0.745 → 0.217), with HD95 rising from ~29px to ~130px, driven by a collapse in recall (0.76 → 0.19) — the model becomes too conservative on the target domain and misses most of the actual blood. For Rabbani→HemoSet the drop is more contained (0.684 → 0.585), but precision collapses instead (0.74 → 0.50) while recall stays high, consistent with the original observation about false positives. The three architectures behave very similarly under every condition, and none of them resolves the domain gap on its own. These baseline numbers are quantitatively consistent with the zero-shot estimates reported in the prior student project mentioned in Section 2 (Dice 0.29 and 0.56 respectively) despite the different splits and hyperparameters used, which is taken here as a sanity check on the measurements rather than a direct comparison.

### 4.2 Data Augmentation, Appearance Adaptation, Joint Training

An "aggressive" augmentation policy (hue/saturation, gamma, blur, noise, JPEG compression, via Albumentations) improves cross-dataset robustness across all three architectures in both directions, at almost no in-domain cost (HemoSet→Rabbani: 0.217 → 0.321, +48% relative; Rabbani→HemoSet: 0.585 → 0.601). Reinhard color transfer and FDA, applied using unlabeled target-domain images, help the more critical direction as well (HemoSet→Rabbani: 0.217 → 0.263 and → 0.242 respectively), but at a non-negligible in-domain cost (Rabbani in-domain: 0.684 → 0.611 with Reinhard), and neither method beats aggressive augmentation overall. A possible explanation is that adaptation, unlike augmentation, is always active during training rather than applied with some probability per image: the model never sees the "clean" appearance of its own source domain, and ends up split between two styles instead of properly consolidating one with variations around it. Fig. 1 shows two independently-sampled draws of the aggressive policy on representative training images, illustrating what is actually being varied beyond what the metrics alone convey.
...
Training on the union of HemoSet and Rabbani (joint training, with separate held-out test sets) was not a solution: there is no systematic benefit on HemoSet, and a systematic drop on Rabbani (0.684 → 0.636) caused by the size imbalance between the two datasets in the combined training set (~59% HemoSet vs ~41% Rabbani). A domain-balanced sampling scheme (`WeightedRandomSampler`, ~50/50 per batch) recovers only about half of the drop (0.636 → 0.656), which is not enough to make joint training competitive with the other strategies.

### 4.3 Ensemble of the Three Architectures

As an additional methodological choice of our own, the three architectures' predictions are ensembled at zero additional training cost, simply by averaging the sigmoid probabilities of the already-trained aggressive-augmentation checkpoints. The result beats the best single model on all four tested conditions, with modest but systematic, exception-free gains (e.g., HemoSet→Rabbani: 0.337 best single model → 0.343 ensemble; Rabbani→HemoSet: 0.615 → 0.623), consistent with the standard ensembling rationale: errors made independently by architecturally different models tend to disagree and partially cancel out when their probabilities are averaged, while the shared, correct signal is reinforced.

A practical caveat is worth noting given this project's real-time motivation (Section 1): the ensemble triples inference cost (three forward passes instead of one) relative to a single model. This is an acceptable trade-off for the offline evaluation performed here, but a deployed real-time bleeding-detection system would need to weigh the modest gains reported above against the added latency, or fall back to aggressive augmentation alone — already the single most effective and cheapest strategy tested — if strict real-time constraints apply.
...
Figures 3 and 4 (see PDF rendering of this report) show representative examples for each cross-dataset direction, comparing the ground-truth mask against the predictions of the source-only baseline, the aggressive-augmentation model, and the ensemble, overlaid on the original image. The baseline under-segmentation on HemoSet→Rabbani (missed blood regions) and the over-segmentation on Rabbani→HemoSet (false-positive regions) are visually apparent, and both are progressively mitigated by aggressive augmentation and further by the ensemble.
...
The common factor behind the two less effective strategies — always-active appearance adaptation and imbalanced joint training — appears to be the lack of a mechanism that preserves the model's exposure to the "clean" distribution of its own source domain during training. A light, probabilistic intervention on visual variety outperforms a more targeted but always-active intervention on style, and it also outperforms simply mixing the data together. This is visually corroborated by the qualitative examples in Figures 3–4: aggressive augmentation and the ensemble progressively recover the recall lost by the source-only baseline on HemoSet→Rabbani, and reduce, without fully eliminating, the over-segmentation observed in the opposite direction.
...
1. **"Blood-index" input channel** (ratio R/(R+G+B), in theory less sensitive to camera/illumination differences): this slightly improves HemoSet→Rabbani (0.321 → 0.331) but markedly worsens Rabbani→HemoSet (0.601 → 0.526), since the index, computed on raw pixels, ends up carrying over the white-balance differences between the datasets instead of ignoring them, becoming a shortcut feature specific to the source domain.
2. **MixStyle**: mixes the encoder's feature-map statistics between samples in the same batch. The result is a diffuse but small degradation in both cross-dataset directions (0.217 → 0.203; 0.585 → 0.561); a plausible reason is that it is applied too early in the encoder, and that training batches remain single-domain in this setting, which limits the actual style diversity injected.
3. **Test-time BatchNorm recalibration** (AdaBN): recalibrates BatchNorm statistics on unlabeled target-domain images at inference time, with no additional training. It shifts the precision/recall trade-off in opposite directions in the two cross-dataset conditions, but net Dice does not improve in either one (0.321 → 0.318; 0.601 → 0.570).

None of the three beats the recommended configuration, although the pattern is informative: three very different mechanisms fail in the same way, either by introducing a shortcut specific to the source domain or by disturbing useful information without replacing it with something more transferable. This reinforces the conclusion that aggressive augmentation combined with ensembling is not a local optimum that can be easily improved on with light interventions, at least for this pair of datasets.

---

## 6. Limitations and Future Work

On the experimental design side, Rabbani's results rest on a single fixed 70/15/15 split rather than a cross-validated one; a classic k-fold cross-validation — a plain random split into k folds, without stratification, since no grouping variable analogous to HemoSet's subjects is available — would provide a variance estimate comparable to HemoSet's 5-fold results, together with a closer look at why one particular HemoSet subject is consistently harder to segment than the rest. On the methodological side, two refinements to the strategies already tested represent natural next steps: applying appearance/domain adaptation with probability <1 instead of always-on, to check whether this alone explains the in-domain cost observed in Section 4.2, and combining it with aggressive augmentation to see whether the two effects compound rather than compete.

Two directions that were not explored trade simplicity for potentially larger gains at the cost of additional complexity: self-training with pseudo-labels on the target domain, and the style-content disentanglement or Transformer-backbone approaches discussed in Sections 2 and 5.2. Finally, moving toward deployment, the ensemble could be extended across folds as well as architectures, and the recommended configuration should be re-evaluated on real TRAMIS data once available, keeping in mind the domain-balancing lesson from the joint-training experiments, since mixing external datasets of very different sizes is not automatically beneficial without it.
